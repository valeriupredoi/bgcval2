...
class depthProfileNC:
    ...
    def run(self):
        if not self.vars:
            print('depthProfileNC:\tERROR:\tvariables to save are no use:',
                  self.vars)
            return
        if not exists(self.fni):
            print('depthProfileNC:\tERROR:\tinputfile name does not exists:',
                  self.fni)
            return
        ...
        if isinstance(self.loc[0], int) and isinstance(self.loc[1], int):
            ...
            if len(nci.variables['latbnd'].shape) == 2:
                #ORCA grid
                la, lo = getNemoIndex(self.loc[0], self.loc[1],
                                      nci.variables['latbnd'],
                                      nci.variables['lonbnd'])
            else:
                #1D latbnd,lonbnd
        ...
        for v in self.vars:
            ...
            print('depthProfileNC:\tERROR:\tvariable,', v, ', not found in ',
                  self.fni)
            nci.close()
            return
        ...
        for var in save:
            ...
            if self.debug:
                print('depthProfileNC:\t Adding long_name for ', var,
                      long_name)
        ...
        for var in save:
            try:
                ...
                print('depthProfileNC:\tWarning:\tNo units for ', var)

        # Fill Values:
        for var in save:
            if self.debug:
                print('depthProfileNC:\tINFO:\tCopying ', var, ' ...')
            ...
            if len(shape) == 4:
                ...
                try:
                    ...
                    print(
                        'depthProfileNC:\tERROR:\tCoping failed due to unusual shape',
                        var, shape, self.slice)
                    return
            elif len(nci.variables[var].shape
                     ) == 2:  # or var in latNames or var in lonNames:
                arr = nci.variables[var][self.slice[0], self.slice[1]]
                arr = arr[None, None]
            else:
            ...
            if self.debug:
                print('depthProfileNC:\tInfo:\tSaving var:',
                      var,
                      arr.shape,
                      end=' ')
                print('[nco is expecting:', nco.variables[var][:].shape,
        ...
        nci.close()
        if self.debug:
            print('depthProfileNC:\tINFO:\tsuccessfully created:\t', self.fno)
        return
...
def getNemoIndex(lat, lon, latbnd, lonbnd):
    ...
    for la in range(a):
        ...
        for lo in range(b):
            ...
            if lob[0] > lob[1] or lob[2] < lob[3]:
                lob = lob + [-360., 0., 0., -360.]
            if not (lob.min() <= lon < lob.max()): continue
            if lat in lab or lon in lob:  #Lat,long matches box exactly:
                print(la, lo, '\ton latbnd,lonbnd edge:', lat, lab, '\t:', lon,
                      lob)
                return la, lo
            if point_inside_polygon(lat, lon, list(zip(lab, lob))):
