...
.. module:: TotalBenCa
   :platform: Unix
   :synopsis: This function calculates the total quantity of seafloor alkalinity for the MEDUSA model on the eORCA grid.
...
def calc_total_benca(nc, keys, **kwargs):
    """
    This function calculates the total quantity of seafloor alkalinity for the MEDUSA model on the eORCA grid.
    """
    ...
    factor =  2. / 1.E18   # mmol CaCO3 / m2 -> Peq
    try:   
        arr = nc.variables[keys[0]][:].squeeze() * factor    # mmol CaCO3 / m2 -> Peq
    except: 
        raise ValueError(f"TotalBenCa: Not able to load {keys[0]}) from {nc.filename}")
